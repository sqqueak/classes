...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    possible = [i for i in range(len(paragraphs)) if select(paragraphs[i])]
    try:
        return paragraphs[possible[k]]
    except IndexError:
        return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(sentence):
        words = split(remove_punctuation(lower(sentence)))
        for t in topic:
            if t in words:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if not typed_words and not reference_words:
        ...
        correct = 0
        for (typed, reference) in zip(typed_words, reference_words): # zip to iterated through 2 lists
            if typed == reference:
                correct += 1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for word in valid_words:
            ...
            if lowest_diff == None or lowest_diff > curr_diff:
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def feline_flips_helper(start, goal, limit, diff):
        if diff > limit:
            return diff
            ...
            if start[0] == goal[0]:
                return feline_flips_helper(start[1:], goal[1:], limit, diff)
            else:
                return 1 + feline_flips_helper(start[1:], goal[1:], limit, diff+1)
    return feline_flips_helper(start, goal, limit, 0)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    def minimum_mewtations_helper(start, goal, changes):
        if changes > limit:
            return limit
        elif start == goal:
            return 0
            ...
            if start[0] == goal[0]:
                return minimum_mewtations_helper(start[1:], goal[1:], changes)
            else:
                add = minimum_mewtations_helper(start[0:], goal[1:], changes + 1)
                remove = minimum_mewtations_helper(start[1:], goal[0:], changes + 1)
                substitute = minimum_mewtations_helper(start[1:], goal[1:], changes + 1)
                return min(add, remove, substitute) + 1

    return minimum_mewtations_helper(start, goal, 0)
...
def time_per_word(words, times_per_player):
    ...
    for player_times in times_per_player:
        ...
        for i in range(len(player_times)-1):
            player.append(player_times[i+1]-player_times[i])
        times.append(player)
...
def fastest_words(match):
    ...
    for w in word_indices:
        #print(word_at(match, w))
        fastest_t = None
        fastest_p = None
        for player in player_indices:
            #print(time(match, player, w))
            curr = time(match, player, w)
            if fastest_t == None or curr < fastest_t:
                fastest_t = curr
                fastest_p = player
        fastest_words[fastest_p].append(get_words(match)[w])

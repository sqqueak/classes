...
class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.funds == 0 or self.funds < self.price:
    ...
    if n < 10:
        ...
        while n != 0:
            if n < 10:
                ...
                remaining += n%10 * 10 ** zeroes
                zeroes += 1
            n //= 10
        #print("DEBUG: ", first, remaining)
        return Link(first, store_digits(remaining))
...
def two_list(vals, amounts):
    ...
    """
    li = []
    for v, a in zip(vals, amounts):
        li += [v] * a

    #print("DEBUG:",li)

    def two_list_helper(list):
        if len(list) == 1:
            return Link(list.pop())
        else:
            return Link(list.pop(0), two_list_helper(list))

    return two_list_helper(li)
...
def is_bst(t):
    ...
    False
    """
    "DKJFLHSDLKJHFLSKJHFKJLSDFHKFLSDHFKJLDSHFL"

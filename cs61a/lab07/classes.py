...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        300
        """
        holder = opponent_card.attack
        opponent_card.attack = opponent_card.defense
        opponent_card.defense = holder
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        for c in player.deck.cards:
            c.attack += opponent_card.attack
            c.defense += opponent_card.defense
        cpy = opponent.deck.cards[:]
        for c in cpy:
            if c is not opponent_card and (c.attack == opponent_card.attack or c.defense == opponent_card.defense):
                opponent.deck.cards.remove(c)
